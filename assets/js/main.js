...
qstart.then(function() {
  ...
  if(!tools.isMobile()) {
    ...
    if(!tools.isIE() && !(!tools.isChrome() && tools.isSafari())) {
    ...
    var tryDemo = document.querySelector('.try-demo');

    tryDemo.addEventListener('click', function(e) {

      e.preventDefault();

      if(!tools.isIE() && !(!tools.isChrome() && tools.isSafari())) {

        analytics.trackStartDemo().then(function() {

          window.location.href = tryDemo.getAttribute('href');

        });

      } else {

        alert('Chrome & Firefox sont les seuls navigateurs supportés par la démo.');
